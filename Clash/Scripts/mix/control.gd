...
const RANDOM_STR_FONT_SIZE = 35  # 设置随机数串的字体大小
const GAME_TIP_FONT_SIZE = 25  # 设置游戏提示的字体大小
...
func _ready():
	...
	game_tip_label.text = "游戏开始！点击数字。\n规则：从第一个数开始，然后随机数串下一个数在该行\n寻找并点击，下一个数在上一个数的所在列寻找并点击，\n下一个再次变为行内寻找，如此往复。"
...
func show_game_over(message: String):
	...
	game_over_label.position = Vector2(100, ROWS * BUTTON_SIZE + 100)
